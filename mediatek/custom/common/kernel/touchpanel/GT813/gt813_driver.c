...
//*/
    ...
    printk("\r\n zhaoshaopeng tpd_i2c_probe success \r\n");
#ifdef ESD_PROTECT
...
#if 0//zhaoshaopeng 20130314 start
...
static int touch_event_handler(void *unused)
{
    ...
    do{
		...
		TPD_DEBUG("[mtk-tpd] KEY STATUS : 0x%x \n", keyStatus);
             //printk("\r\n zhaoshaopeng [mtk-tpd] KEY STATUS : 0x%x \n", keyStatus);
		if ( 0x0F == keyStatus )
        {
			ret = tpd_init_panel();
			ret = i2c_enable_commands( i2c_client, TPD_I2C_DISABLE_REG );
...
//zhaoshaopeng 20130314 start for key process
             if(keyStatus == 0x01)
            {
       		key_info.x[0] = tpd_keys_dim_local[0][0];
       		key_info.y[0] = tpd_keys_dim_local[0][1];
...
//
		...
		if ( 0x00 == keyStatus && 0x00 != lastKey )
        {
			...
			printk("\r\n key up");
			lastKey = 0x00;
			tpd_up(key_info.x[0], key_info.y[0], key_info.x[0], key_info.y[0], 0, 0);
			input_sync(tpd->dev);
			goto Done;
        }
		else if ( 0x00 != keyStatus )
        {
			...
			tpd_down(key_info.x[0], key_info.y[0], key_info.x[0], key_info.y[0], 0, 0);
			input_sync(tpd->dev);
			goto Done;
		}
            //printk("\r\n zhaoshaopeng [mtk-tpd] lastKey : 0x%x \n", lastKey);  
