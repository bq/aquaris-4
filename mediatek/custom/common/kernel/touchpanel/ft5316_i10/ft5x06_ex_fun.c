...
//zhaoshaopeng define this 20120903
#define AUTO_CLB
/*
...
static unsigned char CTPM_5316_FW[] = {
	#include "ft_app.i"
};

static unsigned char CTPM_6206_FW[] = {
	#include "ft_62203_app.i"
};
...
upgrade with *.i file
*/
int fts_ctpm_fw_upgrade_with_i_file_5316(struct i2c_client *client)
{
	u8 *pbt_buf = NULL;
	int i_ret;
	int fw_len = sizeof(CTPM_5316_FW);
	...
	*/
	...
	if (((CTPM_5316_FW[fw_len - 8] ^ CTPM_5316_FW[fw_len - 6]) == 0xFF)
		&& ((CTPM_5316_FW[fw_len - 7] ^ CTPM_5316_FW[fw_len - 5]) == 0xFF)
		&& ((CTPM_5316_FW[fw_len - 3] ^ CTPM_5316_FW[fw_len - 4]) == 0xFF)) 
       {
		/*FW upgrade */
		pbt_buf = CTPM_5316_FW;
		/*call the upgrade function */
		i_ret = fts_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_5316_FW));
		if (i_ret != 0)
			{
...
u8 fts_ctpm_get_i_file_ver_5316(void)
{
	u16 ui_sz;
	ui_sz = sizeof(CTPM_5316_FW);
	if (ui_sz > 2)
		return CTPM_5316_FW[ui_sz - 2];

	return 0x00;	/*default value */
}


int fts_ctpm_fw_upgrade_with_i_file_6x06(struct i2c_client *client)
{
	u8 *pbt_buf = NULL;
	int i_ret;
	int fw_len = sizeof(CTPM_6206_FW);
	...
	*/
	...
	if (((CTPM_6206_FW[fw_len - 8] ^ CTPM_6206_FW[fw_len - 6]) == 0xFF)
		&& ((CTPM_6206_FW[fw_len - 7] ^ CTPM_6206_FW[fw_len - 5]) == 0xFF)
		&& ((CTPM_6206_FW[fw_len - 3] ^ CTPM_6206_FW[fw_len - 4]) == 0xFF)) 
       {
		/*FW upgrade */
		pbt_buf = CTPM_6206_FW;
		/*call the upgrade function */
		i_ret = fts_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_6206_FW));
		if (i_ret != 0)
			{
			dev_err(&client->dev, "%s:upgrade failed. err.\n",__func__);
			}
...
u8 fts_ctpm_get_i_file_ver_6x06(void)
{
	u16 ui_sz;
	ui_sz = sizeof(CTPM_6206_FW);
	if (ui_sz > 2)
		return CTPM_6206_FW[ui_sz - 2];
...
unsigned char cur_ic = IC_FT5316;
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
      ...
      if(id==0xa)//5316
	{
	     ...
	     printk("\r\n i am 5316");
        	...
        	uc_host_fm_ver = fts_ctpm_get_i_file_ver_5316();
              printk("\r\n zhaoshaopeng uc_tp_fm_ver=%x, uc_host_fm_ver=%x", uc_tp_fm_ver ,uc_host_fm_ver);
        	if (/*the firmware in touch panel maybe corrupted */
        		...
        		dev_dbg(&client->dev, "[FTS] uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n",
        				uc_tp_fm_ver, uc_host_fm_ver);
        		i_ret = fts_ctpm_fw_upgrade_with_i_file_5316(client);
        		if (i_ret == 0)	{
        			msleep(300);
        			uc_host_fm_ver = fts_ctpm_get_i_file_ver_5316();
	     ...
	     cur_ic=IC_FT6X06;
	ft5x0x_read_reg(client, FT5x0x_REG_FW_VER, &uc_tp_fm_ver);
        	uc_host_fm_ver = fts_ctpm_get_i_file_ver_6x06();
              printk("\r\n zhaoshaopeng uc_tp_fm_ver=%x, uc_host_fm_ver=%x", uc_tp_fm_ver ,uc_host_fm_ver);
	if (/*the firmware in touch panel maybe corrupted */
		uc_tp_fm_ver == FT5x0x_REG_FW_VER ||
		/*the firmware in host flash is new, need upgrade */
		...
		dev_dbg(&client->dev, "[FTS] uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n",
				uc_tp_fm_ver, uc_host_fm_ver);
        		i_ret = fts_ctpm_fw_upgrade_with_i_file_6x06(client);
		if (i_ret == 0)	{
			msleep(300);
        			uc_host_fm_ver = fts_ctpm_get_i_file_ver_6x06();
			dev_dbg(&client->dev, "[FTS] upgrade to new version 0x%x\n",
...
static void fts_get_upgrade_info(struct Upgrade_Info *upgrade_info)
{
	...
	printk("\r\n zhaoshaopeng 20130320 cur_ic=%x \r\n", cur_ic);
	switch (cur_ic) {		
	case IC_FT5X06:
			  ...
			  u32 dw_lenth)
{
	...
	/*********Step 1:Reset  CTPM *****/
	/*write 0xaa to register 0xfc */
	ft5x0x_write_reg(client, ((cur_ic==IC_FT5316)?0xfc:0xbc), FT_UPGRADE_AA);
	msleep(upgradeinfo.delay_aa);
       
	/*write 0x55 to register 0xfc */
	ft5x0x_write_reg(client, ((cur_ic==IC_FT5316)?0xfc:0xbc), FT_UPGRADE_55);
	...
	if (reg_val[0] == upgradeinfo.upgrade_id_1
		...
		dev_dbg(&client->dev, "[FTS] Step 3: CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n",reg_val[0], reg_val[1]);
		printk("\r\n zhaoshaopeng [FTS] Step 3: CTPM ID,ID1 = 0x%x,ID2 = 0x%x \r\n",reg_val[0], reg_val[1]);
	} else {
		dev_dbg(&client->dev, "[FTS] Step 3: CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n",
			reg_val[0], reg_val[1]);
		printk("\r\n fail [FTS] Step 3: CTPM ID,ID1 = 0x%x,ID2 = 0x%x \r\n",reg_val[0], reg_val[1]);
		return -EIO;
	}
