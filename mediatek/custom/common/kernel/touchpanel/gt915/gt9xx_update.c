 ...
 *      2012/12/12,modify gup_enter_update_mode,rewrite i2c read/write func,check update file checksum
 *
 *
 */
#include "tpd.h"
...
static u8 gup_check_update_file(struct i2c_client *client, st_fw_head *fw_head, u8 *path)
{
    ...
    GTP_DEBUG("Wait data mounted done!");
...
s32 gup_update_proc(void *dir)
{
    ...
    GTP_DEBUG("Wait 3000ms done!");
    GTP_INFO("Wait 3000ms done!");
    if(guitar_client == NULL)                                           //20121212
    {
        guitar_client = i2c_client_point;
    }
    
    GTP_INFO("Set guitar_client done!");
    ret = gup_check_update_file(guitar_client, &fw_head, (u8 *)dir);    //20121212

    if (FAIL == ret)
    {
        GTP_INFO("[update_proc]check update file fail.");
        goto update_fail;
    }
    
    GTP_INFO("Check update file done!");
    gtp_reset_guitar(guitar_client, 20);
    
    GTP_INFO("Reset guitar done!");
    ret = gup_get_ic_fw_msg(guitar_client);
		GTP_INFO("[update_proc]get ic message return vale=%d.",ret);
    if (FAIL == ret)
    {
    ...
    GTP_DEBUG("Get ic fw msg done!");
    ret = gup_enter_upadte_judge(&fw_head);                             //20121212
		GTP_INFO("after gup_enter_upadte_judge return value=%d",ret);
    if (FAIL == ret)
    {
    ...
    GTP_DEBUG("Enter update judge done!");
    GTP_INFO("before gup_enter_update_mode return value=%d",ret);
    ret = gup_enter_update_mode(guitar_client);
		GTP_INFO("after gup_enter_update_mode return value=%d",ret);
    if (FAIL == ret)
    {
    ...
    while (retry++ < 5)
    {
        ...
        ret = gup_burn_dsp_isp(guitar_client);
				GTP_INFO("after gup_burn_dsp_isp return value=%d",ret);
        if (FAIL == ret)
        {
        ...
        ret = gup_burn_fw_ss51(guitar_client);
				GTP_INFO("after gup_burn_fw_ss51 return value=%d",ret);
        if (FAIL == ret)
        {
        ...
        ret = gup_burn_fw_dsp(guitar_client);
				GTP_INFO("after gup_burn_fw_dsp return value=%d",ret);
        if (FAIL == ret)
        {
        ...
        ret = gup_burn_fw_boot(guitar_client);
				GTP_INFO("after gup_burn_fw_boot return value=%d",ret);
        if (FAIL == ret)
        {
    ...
    GTP_INFO("[update_proc]reset chip.");
    gtp_reset_guitar(guitar_client, 20);
    msleep(100);
    GTP_INFO("[update_proc]send config.");
    ...
    GTP_INFO("[update_proc]leave update mode.");
...
download:
    ...
    gtp_reset_guitar(guitar_client, 20);                        //20121212
    ret = gup_get_ic_fw_msg(guitar_client);
    ...
    ret = gup_set_ic_msg(guitar_client, _rRW_MISCTL__SWRST_B0_, 0x04);              //20121212
