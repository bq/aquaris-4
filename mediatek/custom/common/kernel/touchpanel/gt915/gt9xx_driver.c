...
static struct i2c_board_info __initdata i2c_tpd = { I2C_BOARD_INFO("gt9xx", (0xBA>> 1))};//(0xBA>> 1)
...
/* proc file system */
 s32 i2c_read_bytes_9xx(struct i2c_client *client, u16 addr, u8 *rxbuf, int len);
 s32 i2c_write_bytes_9xx(struct i2c_client *client, u16 addr, u8 *txbuf, int len);
...
static s32 tpd_enable_ps(s32 enable)
{
    ...
    ret = i2c_write_bytes_9xx(i2c_client_point, TPD_PROXIMITY_ENABLE_REG, &state, 1);
...
static int gt91xx_config_read_proc(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    ...
    i2c_read_bytes_9xx(i2c_client_point, GTP_REG_CONFIG_DATA, temp_data, GTP_CONFIG_MAX_LENGTH);
...
static int gt91xx_config_write_proc(struct file *file, const char *buffer, unsigned long count, void *data)
{
 ...
 int i2c_read_bytes_9xx(struct i2c_client *client, u16 addr, u8 *rxbuf, int len)
{
    ...
    GTP_DEBUG("i2c_read_bytes_9xx to device %02X address %04X len %d\n", client->addr, addr, len);
...
s32 gtp_i2c_read(struct i2c_client *client, u8 *buf, s32 len)
{
    ...
    ret = i2c_read_bytes_9xx(client, addr, &buf[2], len - 2);
 ...
 int i2c_write_bytes_9xx(struct i2c_client *client, u16 addr, u8 *txbuf, int len)
{
    ...
    GTP_DEBUG("i2c_write_bytes_9xx to device %02X address %04X len %d\n", client->addr, addr, len);
...
s32 gtp_i2c_write(struct i2c_client *client, u8 *buf, s32 len)
{
    ...
    ret = i2c_write_bytes_9xx(client, addr, &buf[2], len - 2);
...
static s32 gtp_init_panel(struct i2c_client *client)
{
    ...
    if(rd_cfg_buf[GTP_ADDR_LENGTH]>5)
    {
    	rd_cfg_buf[GTP_ADDR_LENGTH]=0;
  	}
    ...
    abs_y_max = (config[RESOLUTION_LOC + 3] << 8) + config[RESOLUTION_LOC + 2];
    int_type = (config[TRIGGER_LOC]) & 0x03;
    printk("\r\n zhaoshaopeng abs_x_max=%x, abs_y_max=%x \r\n", abs_x_max, abs_y_max);
...
static s8 gtp_i2c_test(struct i2c_client *client)
{
    ...
    while (retry++ < 5)
    {
        ret = i2c_read_bytes_9xx(client, GTP_REG_HW_INFO, (u8 *)&hw_info, sizeof(hw_info));
        printk("\r\n zhaoshaopeng gtp_i2c_test hw_info=%08x", hw_info);
...
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    //i2c_client_point->addr = 0x5d;//zhaoshaopeng add
    ret = tpd_power_on(client);

    if (ret < 0)
    {
        GTP_ERROR("zhaoshaopeng I2C communication ERROR!");
        return -1;//zhaoshaopeng add if poweron fail just return
    }
...
#if (defined(MT6575)||defined(MT6577)||defined(MT6589))
...
static int touch_event_handler(void *unused)
{
    ...
    u8 proximity_status;
#endif
            printk("zhuoshineng touch_event_handler\n ");
    ...
    do
    {
      ...
      key_value = point_data[3 + 8 * touch_num];
            ...
            printk("zhuoshineng touch_event_handler key_value =%d\n ",key_value);
        ...
        else if (pre_touch)
        {
           ...
           // printk("zhuoshineng touch_event_handler input_x =%d\n ",input_x);
           // printk("zhuoshineng touch_event_handler input_y =%d\n ",input_y);
...
{
            printk("zhuoshineng tpd_local_init\n ");
...
static s8 gtp_enter_sleep(struct i2c_client *client)
{
    ...
    u8 i2c_control_buf[3] = {(u8)(GTP_REG_SLEEP >> 8), (u8)GTP_REG_SLEEP, 5};
            printk("zhuoshineng gtp_enter_sleep\n ");
...
static s8 gtp_wakeup_sleep(struct i2c_client *client)
{
    ...
    s8 ret = -1;
            ...
            printk("zhuoshineng gtp_wakeup_sleep\n ");
...
static void tpd_suspend(struct early_suspend *h)
{
    s32 ret = -1;
    tpd_halt = 1;
            printk("zhuoshineng tpd_suspend\n ");
