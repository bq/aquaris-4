...
extern void	arch_clean_invalidate_cache_range(unsigned int start, unsigned int len);
...
DSI_STATUS DSI_PowerOff(void)
{
    ...
    if (s_isDsiPowerOn)
    {
		...
		printf("[DISP] - DSI_PowerOff. 0x%8x,0x%8x,0x%8x\n", INREG32(0xC2080000), INREG32(0xC2080004), INREG32(0xC2080008));
#endif        
...
//zhuoshineng add
void DSI_set_cmdq_dcs(unsigned cmd, unsigned char count, unsigned char *para_list, unsigned char force_update)
{
	UINT32 i, layer, layer_state, lane_num;
	UINT32 goto_addr, mask_para, set_para;
	UINT32 fbPhysAddr, fbVirAddr;
	...
	if (count > 59)
	{
		DISP_LOG_PRINT(ANDROID_LOG_WARN, "huangzs", "count > 59  cmd:0x%x\n", cmd);
...
//	else
	...
	DISP_LOG_PRINT(ANDROID_LOG_WARN, "huangzs", "Data_ID:0x%x data:0x%x\n", t0.Data_ID, t0.Data0);
