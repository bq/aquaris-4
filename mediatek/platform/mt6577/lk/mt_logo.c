...
//	DISP_PROG_BAR_DIRECT direct = DISP_VERTICAL_PROG_BAR;
        ...
        if (capacity <= 0) 
          {
            ...
            //   {
                  ...
                  // fenggy show_animation_number(number_pic_start_0,1);
                  // fenggyshow_animation(14, percent_location_rect, percent_pic_addr);
        ...
        if (capacity < 10 && capacity > 0) 
          {
             ...
             //fenggyshow_animation_number(number_pic_start_0+capacity,1);
             // fenggy show_animation(14, percent_location_rect, percent_pic_addr);
             mt_disp_update(0, 0, CFG_DISPLAY_WIDTH, CFG_DISPLAY_HEIGHT);	
             return;
          }
	...
	UINT32 capacity_grids = 0;     
	//if (capacity > 100) capacity = 100;
    capacity_grids = battery_rect.bottom - (battery_rect.bottom - battery_rect.top) * (capacity - 10) / 99;//zhaoshaopeng change 90 to 99
	printf("[ChargingAnimation]capacity : %d\n", capacity);
    ...
    if (capacity <= 99)//zhaoshaopeng from 90
          {
