...
SPELayer::SPELayer()
{
	...
	mRecordPGAGain = 16;	//MIC_DG for AGC
	...
	mVoIPPGAGain = 16;	//MIC_DG  for AGC
...
//	Mutex::Autolock lock(mLock);
	...
	{
#ifdef MTK_AP_SPEECH_ENHANCEMENT_VOIP
        //mVoIPPGAGain = mAudioParamVoIP.PGA_gain_param[mRoute];
...
bool	SPELayer::SetPGAGain(SPE_MODE mode, long PGA_gain)
{
	switch(mode)
	{
		case SPE_MODE_REC:				
			mRecordPGAGain = PGA_gain;
			break;
		case SPE_MODE_VOIP:
			mVoIPPGAGain = PGA_gain;
			break;
		default:
			ALOGD("SPELayer::SetPGAGain, not support mode");
			return false;
			break;
	}
	
	ALOGD("SPELayer::SetPGAGain MIC_DG, SPE_MODE=%d, gain=%d",mode,PGA_gain);
...
long	SPELayer::GetPGAGain(SPE_MODE mode)
{
	long retPGAGain = 0;
	switch(mode)
	{
		case SPE_MODE_REC:
			retPGAGain = mRecordPGAGain;
			break;
		case SPE_MODE_VOIP:
			retPGAGain = mVoIPPGAGain;
			break;
		default:
	...
	ALOGD("SPELayer::GetPGAGain, SPE_MODE=%d, retPGAGain=%d",mode,retPGAGain);
	return retPGAGain;
}
...
bool    SPELayer::Start(SPE_MODE mode)  //for VOIP, both uplink/downlink
{
	...
	if(mode == SPE_MODE_REC)
	{
	    ...
	    mSph_Enh_ctrl.MIC_DG = mRecordPGAGain;
	...
	else if(mode == SPE_MODE_VOIP)
	{
	    ...
	    mSph_Enh_ctrl.MIC_DG = mVoIPPGAGain;
    ...
    ALOGD("SPELayer::going to configure,mSphCtrlBuffer=%p",mSphCtrlBuffer);
	...
	{   //only support 16K samplerate
        ...
        mpSPEBufferNE = &mSph_Enh_ctrl.ne_out[0];
        mpSPEBufferFE = &mSph_Enh_ctrl.fe_out[0];
	}
