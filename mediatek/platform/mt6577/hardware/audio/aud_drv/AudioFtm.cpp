...
bool AudioFtm::SetLoopbackOn(ENUM_LOOPBACK_TYPES eLoopbackType, bool bEarphoneInserted)
{
    ...
    // set gain
    //modify by lidandan for audio loopback gain start
    if(bEarphoneInserted)
    {
    	 mLad->LAD_SetOutputVolume(0xFFFFC0);     // 1 db range(-32~12)
    	 mLad->LAD_SetMicrophoneVolume(0xFFFFC0); // the speech gain
    }
    else
    {
    	 mLad->LAD_SetOutputVolume(0xFFFFFF);     // 1 db range(-32~12)
       mLad->LAD_SetMicrophoneVolume(0xFFFFE2); // the speech gain
    }
    //modify by lidandan for audio loopback gain end
    mLad->LAD_SetSidetoneVolume(0xFFF6AD);   // the lowest gain
